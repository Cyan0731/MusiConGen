...
class MusicGenSolver(base.StandardSolver):
    ...
    def build_model(self) -> None:
        ...
        for name, param in self.model.named_parameters():
            ...
            if name.startswith("out_norm") or name.startswith("linears"):
                    param.requires_grad = True
            if name.startswith("condition_provider.conditioners.chord") or name.startswith("condition_provider.conditioners.beat"):
                    param.requires_grad = True
            # if name.startswith("condition_provider.conditioners.beat"):
            #         param.requires_grad = True
    ...
    def run_step(self, idx: int, batch: tp.Tuple[torch.Tensor, tp.List[SegmentWithAttributes]], metrics: dict) -> dict:
        ...
        if self.is_training:
            ...
            # apply grad accum
            loss = loss / self.cfg.optim.grad_accum_steps
            if self.cfg.fsdp.use:
                loss.backward()
            ...
            if idx % self.cfg.optim.grad_accum_steps == 0:
                if self.scaler is not None:
